...
void LcdDisplay::ShowStandbyScreen() {
    ...
    // 创建待机界面（如果还没有）
    if (!standby_screen_) {
        standby_screen_ = std::make_unique<StandbyScreen>(width_, height_);
    }
    ...
    // 显示待机界面
    standby_screen_->Show();
    is_standby_mode_ = true;
    
    ESP_LOGI(TAG, "Standby screen shown");
}
...
void LcdDisplay::HideStandbyScreen() {
    ...
    is_standby_mode_ = false;
    
    ESP_LOGI(TAG, "Standby screen hidden");
}

void LcdDisplay::UpdateStandbyTemperatureHumidity(float temperature, float humidity) {
    if (standby_screen_) {
        standby_screen_->UpdateTemperatureHumidity(temperature, humidity);
    }
}
